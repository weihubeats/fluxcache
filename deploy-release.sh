...
TOKEN_HTTP=$(curl -s --max-time 30 -o /tmp/fluxcache-token-check.json -w "%{http_code}" -u "${CENTRAL_USER}:${CENTRAL_PASS}" "https://central.sonatype.com/api/v1/publisher/status")
if [ "$TOKEN_HTTP" = "401" ] || grep -q "Invalid token" /tmp/fluxcache-token-check.json 2>/dev/null; then
